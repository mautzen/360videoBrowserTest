...
var ASM_CONSTS = {
  4512320: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4512381: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4512445: function() {return Module.webglContextAttributes.powerPreference;},  
 4512503: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4512558: function($0) {performance.now = function() { return $0; };},  
 4512606: function($0) {performance.now = function() { return $0; };},  
 4512654: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _IsDeviceiOS() {
          var isios = false;
          try {
              var uaSource = (navigator.userAgent || navigator.vendor || "");
              var ua = uaSource ? uaSource.toLowerCase() : "";
  
              if (ua.indexOf("iphone") !== -1 || ua.indexOf("ipad") !== -1 || ua.indexOf("ipod") !== -1) {
                  isios = true;
              } else {
                  var platform = navigator.platform || "";
                  var isMac = (platform && platform.length >= 3 && platform.substring(0,3) === "Mac");
                  var mtp = (typeof navigator.maxTouchPoints === "number") ? navigator.maxTouchPoints : 0;
                  if (isMac && mtp > 1) isios = true; // iPadOS spoof (MacIntel + touch)
              }
          } catch (e) {
              isios = false;
          }
  ...
  var R4C_GYRO__state = {hasPermission:false,started:false,hasData:false,yawDeg:0,pitchDeg:0,lastAlpha:null,lastBeta:null,lastGamma:null,lastCompass:null,lastAbsoluteAlpha:null,yawSrc:"none"};
  function _R4C_Gyro_GetABG(alphaPtr, betaPtr, gammaPtr) {
      var s = R4C_GYRO__state;
      if (alphaPtr) setValue(alphaPtr, isFinite(s.lastAlpha) ? s.lastAlpha : NaN, 'double');
      if (betaPtr)  setValue(betaPtr,  isFinite(s.lastBeta)  ? s.lastBeta  : NaN, 'double');
      if (gammaPtr) setValue(gammaPtr, isFinite(s.lastGamma) ? s.lastGamma : NaN, 'double');
    }

  function _R4C_Gyro_GetHeading() {
      var s = R4C_GYRO__state;
      if (isFinite(s.lastCompass)) return s.lastCompass;
      if (isFinite(s.lastAbsoluteAlpha)) return s.lastAbsoluteAlpha;
      if (isFinite(s.lastAlpha)) return s.lastAlpha;
      return NaN;
    }

  function _R4C_Gyro_GetPitch() { return R4C_GYRO__state.pitchDeg; }

  function _R4C_Gyro_GetYaw() { return R4C_GYRO__state.yawDeg; }

  function _R4C_Gyro_GetYawSource() {
      var s = R4C_GYRO__state;
      var m = { 'none':0, 'compass':1, 'absAlpha':2, 'alpha':3, 'gamma':4 };
      return m[s.yawSrc] || 0;
    }

  function _R4C_Gyro_HasData() { return R4C_GYRO__state.hasData ? 1 : 0; }

  function R4C_GYRO__normalize180(a) {
      a = a % 360.0;
      if (a > 180.0) a -= 360.0;
      if (a < -180.0) a += 360.0;
      return a;
  ...
  function R4C_GYRO__isLandscape() {
      try {
      ...
      var o = (typeof window !== 'undefined' && typeof window.orientation === 'number') ? window.orientation : 0;
      return o === 90 || o === -90;
    }
  
  function R4C_GYRO__isLandscapeLeft() {
      try {
        if (typeof screen !== 'undefined' && screen.orientation && typeof screen.orientation.angle === 'number') {
          var a = screen.orientation.angle; // 0, 90, 180, 270
          return a === 90;
        }
      } catch (e) {}
      var o = (typeof window !== 'undefined' && typeof window.orientation === 'number') ? window.orientation : 0;
      return o === 90; // iOS convention
    }
  function R4C_GYRO__computeYawPitch(ev) {
      var s = R4C_GYRO__state;
  
      var alpha   = (ev && typeof ev.alpha  === 'number' && isFinite(ev.alpha))  ? ev.alpha  : NaN;
      var beta    = (ev && typeof ev.beta   === 'number' && isFinite(ev.beta))   ? ev.beta   : NaN;
      var gamma   = (ev && typeof ev.gamma  === 'number' && isFinite(ev.gamma))  ? ev.gamma  : NaN;
      var compass = (ev && typeof ev.webkitCompassHeading === 'number' && isFinite(ev.webkitCompassHeading)) ? ev.webkitCompassHeading : NaN;
      var absAlpha = (ev && ev.absolute === true && typeof ev.alpha === 'number' && isFinite(ev.alpha)) ? ev.alpha : NaN;
  
      // Telemetry
      s.lastAlpha = alpha;
      s.lastBeta = beta;
      s.lastGamma = gamma;
      s.lastCompass = compass;
      s.lastAbsoluteAlpha = absAlpha; // clear to NaN when not provided
  
      var isLand = R4C_GYRO__isLandscape();
      var left = R4C_GYRO__isLandscapeLeft();
  
      // --- YAW --- (Unity-friendly sign)
      var yaw;
      if (isFinite(compass)) {
        yaw = -compass;            // iOS compass: clockwise -> negate
        s.yawSrc = 'compass';
      } else if (isFinite(absAlpha)) {
        yaw = -absAlpha;           // absolute alpha clockwise -> negate
        s.yawSrc = 'absAlpha';
      } else if (isFinite(alpha)) {
        yaw = -alpha;              // generic alpha clockwise -> negate
        s.yawSrc = 'alpha';
      } else {
        var g = gamma;
        yaw = (left ? -g : g) || 0.0; // fallback
        s.yawSrc = 'gamma';
      }
  
      // --- PITCH --- prefer gamma in landscape, beta in portrait
      var pitch;
      if (isLand) {
        // Use gamma for up/down; flip sign depending on left/right so "tilt top edge away => look up" feels natural.
        var g2 = gamma || 0.0;
        pitch = left ? g2 : -g2;
      } else {
        pitch = beta || 0.0; // portrait
      }
  
      return {
        yawDeg: R4C_GYRO__normalize180(yaw || 0.0),
        pitchDeg: R4C_GYRO__normalize180(pitch || 0.0)
      };
    }
  function R4C_GYRO__onOrientation(ev) {
      var s = R4C_GYRO__state;
      var ypp = R4C_GYRO__computeYawPitch(ev);
      s.yawDeg = ypp.yawDeg;
      s.pitchDeg = ypp.pitchDeg;
      s.hasData = true;
    }
  function R4C_GYRO__addOrientationListeners() {
      var s = R4C_GYRO__state;
      if (s.started) return;
      if (typeof window !== 'undefined') {
        window.addEventListener('deviceorientation', R4C_GYRO__onOrientation, true);
        try { window.addEventListener('deviceorientationabsolute', R4C_GYRO__onOrientation, true); } catch (e) {}
        var noop = function(){};
        window.addEventListener('orientationchange', noop, true);
        try {
          if (typeof screen !== 'undefined' && screen.orientation && typeof screen.orientation.addEventListener === 'function') {
            screen.orientation.addEventListener('change', noop);
          }
        } catch (e) {}
      }
      s.started = true;
    }
  function R4C_GYRO__ensurePermissionWithFallbackButton() {
      var s = R4C_GYRO__state;
      var needsPermission =
        (typeof DeviceMotionEvent !== 'undefined' && typeof DeviceMotionEvent.requestPermission === 'function') ||
        (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function');
  
      if (!needsPermission) { s.hasPermission = true; return; }
  
      var tryRequest = function () {
        var reqs = [];
        try { if (typeof DeviceMotionEvent !== 'undefined' && typeof DeviceMotionEvent.requestPermission === 'function') { reqs.push(DeviceMotionEvent.requestPermission().catch(function(){ return 'denied'; })); } } catch (e) {}
        try { if (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') { reqs.push(DeviceOrientationEvent.requestPermission().catch(function(){ return 'denied'; })); } } catch (e) {}
        if (reqs.length === 0) { s.hasPermission = true; return Promise.resolve(true); }
        return Promise.all(reqs).then(function (results) {
          var granted = false; for (var i=0;i<results.length;i++){ if (results[i]==='granted'){ granted=true; break; } }
          s.hasPermission = granted; if (granted) R4C_GYRO__addOrientationListeners(); return granted;
        }).catch(function(){ s.hasPermission = false; return false; });
      };
  
     tryRequest().then(function(granted){
      // Unity will show its own UI when not granted.
      return;
  });
    }
  function _R4C_Gyro_RequestPermissionIfNeeded() { R4C_GYRO__ensurePermissionWithFallbackButton(); }

  function _R4C_Gyro_Start() { R4C_GYRO__addOrientationListeners(); }

  function _R4C_Gyro_Supported() {
      return (typeof window !== 'undefined' && 'DeviceOrientationEvent' in window) ? 1 : 0;
    }
...
var asmLibraryArg = {
  ...
  "GetWebGLMemoryMB": _GetWebGLMemoryMB,
  "IsDeviceiOS": _IsDeviceiOS,
  ...
  "R4C_Gyro_GetABG": _R4C_Gyro_GetABG,
  "R4C_Gyro_GetHeading": _R4C_Gyro_GetHeading,
  "R4C_Gyro_GetPitch": _R4C_Gyro_GetPitch,
  "R4C_Gyro_GetYaw": _R4C_Gyro_GetYaw,
  "R4C_Gyro_GetYawSource": _R4C_Gyro_GetYawSource,
  "R4C_Gyro_HasData": _R4C_Gyro_HasData,
  "R4C_Gyro_RequestPermissionIfNeeded": _R4C_Gyro_RequestPermissionIfNeeded,
  "R4C_Gyro_Start": _R4C_Gyro_Start,
  "R4C_Gyro_Supported": _R4C_Gyro_Supported,
...
unexportedRuntimeFunction('R4C_GYRO__state', false);
unexportedRuntimeFunction('R4C_GYRO__state__user', false);
unexportedRuntimeFunction('R4C_GYRO__normalize180', false);
unexportedRuntimeFunction('R4C_GYRO__normalize180__user', false);
unexportedRuntimeFunction('R4C_GYRO__isLandscape', false);
unexportedRuntimeFunction('R4C_GYRO__isLandscape__user', false);
unexportedRuntimeFunction('R4C_GYRO__isLandscapeLeft', false);
unexportedRuntimeFunction('R4C_GYRO__isLandscapeLeft__user', false);
unexportedRuntimeFunction('R4C_GYRO__computeYawPitch', false);
unexportedRuntimeFunction('R4C_GYRO__computeYawPitch__user', false);
unexportedRuntimeFunction('R4C_GYRO__onOrientation', false);
unexportedRuntimeFunction('R4C_GYRO__onOrientation__user', false);
unexportedRuntimeFunction('R4C_GYRO__addOrientationListeners', false);
unexportedRuntimeFunction('R4C_GYRO__addOrientationListeners__user', false);
unexportedRuntimeFunction('R4C_GYRO__ensurePermissionWithFallbackButton', false);
unexportedRuntimeFunction('R4C_GYRO__ensurePermissionWithFallbackButton__user', false);
